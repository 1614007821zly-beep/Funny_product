...
export default function Home() {
  ...
  async function createSharedSchedule() {
    ...
    try {
      ...
      setSelectedDay(localDate(scheduleDraft.date).getDate()); setMonthOffset(0); setAdopted(true); setPartnerAccepted(false); setCompleted(false); setMyConfirmed(false); setTaConfirmed(false); setTaskLinked(taskAccepted); go("schedule");
          ...
          <div className={`screen screen-${screen}`}>
            ...
            {screen === "home" && (
              <div className="page tab-page">
                ...
                <section className="content-section"><div className="section-heading"><div><p className="kicker">下一件小事</p><h3>{adopted ? (scheduleDraft.title || currentPlan.title) : "今晚，想一起做点什么？"}</h3></div><span aria-hidden="true">→</span></div>
                  {adopted ? <button className="event-card" onClick={() => go("schedule")}><span className="date-block"><b>{String(eventDate.getDate()).padStart(2,"0")}</b><small>{eventWeekday}</small></span><span><b>{scheduleDraft.time} · {scheduleDraft.city || profile.city}</b><small>{partnerAccepted ? "双方已接受" : isScheduleCreator ? "等待 TA 接受" : "待你确认"}</small></span><i aria-hidden="true">›</i></button> : <button className="inspiration-card" onClick={() => go("inspire")}><div className="spark" aria-hidden="true">✦</div><div><b>获取一份约会灵感</b><small>告诉我们此刻的心情，剩下的交给灵感</small></div><i aria-hidden="true">›</i></button>}
                </section>
            ...
            {screen === "schedule" && (
              <div className="page schedule-page">
                ...
                <div className="people-row"><div className="avatar a">{profile.name.slice(0,1)}</div><div><b>{cancelled ? "这次安排已取消" : !partnerAccepted ? (isScheduleCreator ? "你已发出邀请" : "TA 已发出邀请") : myConfirmed ? "你已确认完成" : "双方已经接受"}</b><p>{!partnerAccepted ? (isScheduleCreator ? "等待 TA 接受、拒绝或提出修改" : "接受后会进入双方共同日历") : taConfirmed ? "TA 也已确认完成" : myConfirmed ? "正在等待 TA 确认完成" : "安排已进入双方共同日历"}</p></div><div className="avatar b">{partnerProfile.name.slice(0,1)}</div></div>
