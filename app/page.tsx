...
export default function Home() {
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("love-diary-v14");
    if (!saved) return;
    try {
      const data = JSON.parse(saved);
      setProfile(data.profile ?? profile); setChoices(data.choices ?? choices); setAdopted(Boolean(data.adopted));
      setCompleted(Boolean(data.completed)); setTaskAccepted(Boolean(data.taskAccepted)); setTaskLinked(Boolean(data.taskLinked));
  ...
  useEffect(() => {
    window.localStorage.setItem("love-diary-v14", JSON.stringify({ profile, choices, adopted, completed, taskAccepted, taskLinked, memoryCreated, importantAdded, hasStarted }));
  }, [profile, choices, adopted, completed, taskAccepted, taskLinked, memoryCreated, importantAdded, hasStarted]);
