...
type Plan = { eyebrow: string; title: string; meta: string; desc: string; tone: string; duration?: string; timeline?: TimelineNode[]; places?: Place[]; includedPlaces?: Place[]; estimatedCost?: number | null; budgetMatch?: "matched" | "unknown" | "under"; searchRadius?: number; distanceVerified?: boolean };
          ...
          <div className={`screen screen-${screen} ${hasRelationship?"":"solo-mode"}`}>
            ...
            {screen === "inspire" && (
              <div className="page tab-page form-page">
                ...
                <div className="form-intro"><p className="kicker">{hasRelationship?"此刻的你们":"此刻的我"}</p><h2>{hasRelationship?<>今天想和 TA<br/>怎么度过？</>:<>今天想<br/>怎么度过？</>}</h2><p>告诉我此刻的状态、时间与预算，我会从附近真实地点中整理 1 个主方案和 2 个备选。这里只提供建议，确认前不会进入日历。</p></div>
                <details className="inspiration-guide"><summary><span><b>灵感是怎么生成的？</b><small>先看简短说明，需要时再展开</small></span><i><b>展开了解</b><em>收起</em></i></summary><div><section><b>先理解你的条件</b><p>状态、时间、预算、活动空间和需要特别照顾的事项，会共同影响推荐；选填内容留空时不会出现在后续方案中。</p></section><section><b>再筛选真实地点</b><p>系统会按城市、商圈和搜索范围查询高德地点，排除超出范围、明显超预算或已经关闭的地点，再由 AI 组合成 1 主 + 2 备选。</p></section><section><b>最后由你决定</b><p>AI 灵感不会自动成为正式安排。只有你主动采用、确认日期和时间后，才会写入日历；共同安排仍需双方分别确认。</p></section><p className="guide-note">定位只用于本次附近搜索；营业时间、价格、排队和路线可能变化，出发前仍需在高德地图中复查。</p></div></details>
...
function PlaceCandidates({ places, selectedId, onSelect }: { places: Place[]; selectedId?: string; onSelect: (index: number) => void }) {
  ...
  return <section className="place-candidates" aria-labelledby="place-candidates-title"><div className="section-heading"><div><p className="kicker">{places.length>1?"附近候选":"方案地点"}</p><h3 id="place-candidates-title">{places.length>1?"选择更合适的地点":"本方案的主地点"}</h3></div><span>{places.length} 个真实地点</span></div>{places.length ? places.map((place, index) => <button type="button" key={place.id} className={selectedId === place.id ? "selected" : ""} aria-pressed={selectedId === place.id} onClick={() => onSelect(index)}><div><b>{place.name}</b><small title={place.businessArea || place.address}>{place.businessArea || place.address}</small><em>{place.recommendationReasons.slice(0,2).join(" · ")}</em></div><span>{place.distance !== null ? `${distanceFormatter.format(place.distance / 1000)}\u00a0公里` : "距离待定位"}</span></button>) : <p className="candidate-empty">附近没有匹配到地点，请扩大范围或调整商圈。</p>}</section>;
}
