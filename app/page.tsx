...
export default function Home() {
  ...
  async function loadAccount(silent = false) {
    try {
      ...
      if (response.status === 401) { setAccount({ authenticated: false }); setSharedSchedule(null); setSchedules([]); setScheduleLoaded(true); setAdopted(false); return null; }
      if (!response.ok) throw new Error("账号状态读取失败");
      if (account?.user?.id && data.user?.id && account.user.id !== data.user.id) {
        setSharedSchedule(null); setSchedules([]); setScheduleLoaded(false); setAdopted(false);
      }
      setAccount(data);
  ...
  async function generate(shouldFail = false) {
    ...
    try {
      const response = await fetch("/api/inspiration", {
        ...
        body: JSON.stringify({ city: profile.city, moods: myStates, partnerMood: choices.taMood, vibe: choices.vibe, time: choices.time, budget: choices.budget, space: choices.space, special: choices.special.trim(), district: locationPrefs.district, radius: locationPrefs.radius, longitude: locationPrefs.longitude, latitude: locationPrefs.latitude }),
  ...
  return (
    <main className="prototype-shell" id="main-content">
      <aside className="prototype-notes">
        ...
        <p className="kicker">恋爱日记 · V1.15 个人与共同空间</p>
        ...
        <p className="hint">V1.15 由本人决定个人计划是否进入共同空间，历史内容不会自动共享。</p>
      </aside>
          ...
          <div className={`screen screen-${screen} ${hasRelationship?"":"solo-mode"}`}>
            ...
            {screen === "calendar" && (
              <div className="page tab-page calendar-page">
                ...
                <section className="day-agenda"><p className="kicker">{calendarMonth+1}月{selectedDay}日</p>{(()=>{const dateKey=`${calendarYear}-${String(calendarMonth+1).padStart(2,"0")}-${String(selectedDay).padStart(2,"0")}`;const daySchedules=schedules.filter(schedule=>schedule.event_date===dateKey);if(daySchedules.length)return <div className="agenda-list">{daySchedules.map(schedule=><button key={schedule.id} className={`agenda-item ${schedule.visibility==="personal"?"personal-agenda":""}`} onClick={()=>openSchedule(schedule)}><i aria-hidden="true"/><span><b>{schedule.event_time}</b><small>{schedule.visibility==="shared"?"共同安排":"我的计划"}</small></span><div><b>{schedule.title}</b><small>{schedule.visibility==="shared"?(schedule.status==="confirmed"?"双方已接受":"等待确认"):"仅自己可见"} · {schedule.city}</small></div><em aria-hidden="true">›</em></button>)}</div>;if(isImportantMonth&&selectedDay===importantDate.getDate()&&importantAdded)return <button className="agenda-item important-agenda" onClick={() => go("important")}><i aria-hidden="true"/><span><b>全天</b></span><div><b>我们的一周年</b><small>重要日子 · 每年重复</small></div><em aria-hidden="true">›</em></button>;if(isIdeaMonth&&selectedDay===16&&hasGenerated)return <div className="idea-day"><span aria-hidden="true">✦</span><div><b>AI 轻量建议</b><p>周日下午适合去城市周边走走，尚未成为正式安排。</p></div><button onClick={()=>go("inspire")}>继续规划</button></div>;return <div className="empty-day"><span aria-hidden="true">☼</span><p>这一天还没有{hasRelationship?"共同安排或个人计划":"个人计划"}</p><button onClick={() => go("inspire")}>找点灵感</button></div>;})()}</section>
                {bottomNav("calendar")}
              </div>
            )}
