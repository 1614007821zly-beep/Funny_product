...
/* Modal backdrops intentionally use click-to-dismiss while their dialogs stop propagation. */
...
export default function Home() {
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("love-diary-v18") ?? window.localStorage.getItem("love-diary-v17") ?? window.localStorage.getItem("love-diary-v16") ?? window.localStorage.getItem("love-diary-v15") ?? window.localStorage.getItem("love-diary-v14");
    if (!saved) return;
    try {
  ...
  useEffect(() => {
    window.localStorage.setItem("love-diary-v18", JSON.stringify({ profile, partnerProfile, choices, myStates, customStates, adopted, partnerAccepted, hasGenerated, completed, taskAccepted, taskLinked, memoryCreated, importantAdded, hasStarted, memoryContentRetracted, relationshipExited, safetyExitUsed }));
  ...
  return (
    <main className="prototype-shell" id="main-content">
      <aside className="prototype-notes">
        ...
        <p className="kicker">恋爱日记 · V1.8 正式体验版</p>
        ...
        <p className="hint">V1.8 新增关系安全退出、内容撤回与只读归档：离开不需要许可，自己的敏感内容可以立即撤回。</p>
      </aside>
      ...
      <section className="phone-stage">
        <div className="phone">
          ...
          <div className={`screen screen-${screen}`}>
            ...
            {screen === "memoryCreate" && <div className="page formal-page create-memory-page"><header><Back onClick={() => back("memories")}/><span>添加回忆</span><i/></header><section className="page-intro compact"><p className="kicker">一条简单记录也已经完整</p><h2>把这件一起经历的事，<br/>留在这里。</h2></section><section className="create-form"><label>回忆名称 <em>必填</em><input name="memory-title" autoComplete="off" defaultValue="雨天一起逛书店"/></label><label>日期 <em>必填</em><input name="memory-date" type="date" autoComplete="off" defaultValue="2026-08-06"/></label><label>地点 <small>选填，属于共同事实</small><input name="memory-place" autoComplete="off" defaultValue={profile.city}/></label><button className="photo-upload" onClick={() => notify("照片将标记为由你上传；可随时撤回对方访问权限")}><span>＋</span>添加我的照片（最多9张）</button><label>写点什么 <small>选填，由我管理</small><textarea name="memory-copy" autoComplete="off" defaultValue="下雨以后临时改变计划，一起在书店待了很久。"/></label></section><p className="ownership-note">照片和文字由你控制；共同日期与地点由双方各自保留。离线副本无法由平台远程删除。</p><div className="similar-warning"><b>这一天已有一条相似回忆</b><p>系统不会自动合并，你仍然可以创建。</p><button onClick={() => go("memories")}>查看已有回忆</button></div><button className="primary-button" onClick={() => {setMemoryCreated(true);notify("回忆已保存，并记录了内容来源");go("memories");}}>仍然创建并保存 <Arrow/></button></div>}

            {screen === "profile" && <div className="page formal-page"><header><Back onClick={() => back("home")}/><span>我们的资料</span><button className="text-button" onClick={() => setPanel("profileEdit")}>编辑</button></header><section className="profile-hero"><div className="connect-visual"><div className="avatar a">{profile.name.slice(0,1)}</div><span>♡</span><div className="avatar b">{partnerProfile.name.slice(0,1)}</div></div><h2>{profile.name} & {partnerProfile.name}</h2><p>共同空间已连接 · 双方分别管理个人资料</p></section><section className="info-group"><p className="group-label">关系资料</p><InfoRow label="在一起纪念日" value={importantAdded?"2026年10月26日":"尚未添加"}/><InfoRow label="当前城市" value={profile.city}/><InfoRow label="关系状态" value="已连接"/></section><section className="info-group"><p className="group-label">我的资料</p><InfoRow label="昵称" value={profile.name}/><InfoRow label="生日" value={profile.birthday||"未填写"}/></section><section className="info-group"><p className="group-label">TA 已确认的资料</p><InfoRow label="昵称" value={partnerProfile.name}/><InfoRow label="生日" value={partnerProfile.birthday||"由 TA 决定是否共享"}/><p className="partner-note">TA 的资料只能由 TA 修改；你可以请求更正，但不能代为覆盖。</p></section><button className="secondary-button safety-entry" onClick={() => go("relationshipSafety")}>管理关系与数据安全 <Arrow/></button></div>}
