          ...
          <div className={`screen screen-${screen} ${hasRelationship?"":"solo-mode"}`}>
            ...
            {screen === "schedule" && scheduleLoaded && sharedSchedule && (
              <div className="page schedule-page">
                ...
                <section className="schedule-card"><div><span className="label">时间</span><b>{eventMonthDay} {scheduleDraft.time}</b></div><div><span className="label">集合</span><b>近江地铁站 B 口</b></div><div><span className="label">天气</span><b>{scheduleWeather?weatherLabel(scheduleWeather):"仅显示未来 4 天预报"}</b></div><div><span className="label">预算</span><b>约 {eventBudget}{scheduleIsShared?" / 两人":" · 参考"}</b></div><button onClick={() => go("plan")}>查看完整路线 <span>›</span></button></section>
