...
export default function Home() {
  ...
  const restDays = monthOffset === 0 ? [1,2,8,15,16,22,23,29,30] : monthOffset < 0 ? [4,5,11,12,18,19,25,26] : [5,6,12,13,19,20,26,27];
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("love-diary-v16") ?? window.localStorage.getItem("love-diary-v15") ?? window.localStorage.getItem("love-diary-v14");
    if (!saved) return;
    try {
  ...
  useEffect(() => {
    window.localStorage.setItem("love-diary-v16", JSON.stringify({ profile, partnerProfile, choices, myStates, customStates, adopted, partnerAccepted, hasGenerated, completed, taskAccepted, taskLinked, memoryCreated, importantAdded, hasStarted }));
  ...
  return (
    <main className="prototype-shell">
      ...
      <section className="phone-stage">
        <div className="phone">
          ...
          <div className={`screen screen-${screen}`}>
            ...
            {screen === "profileSetup" && <div className="page formal-page onboarding-page"><header><Back onClick={()=>back("age")}/><span>双方基础资料</span><i/></header><section className="page-intro compact"><p className="kicker">从两个人开始</p><h2>让共同空间，<br/>有双方真实的称呼。</h2><p className="confirm-copy">你可以先代填伴侣的基础资料；真实双人版本连接后，TA 可以自行确认和修改。</p></section><section className="create-form"><p className="form-section-label">我的资料</p><label>我的昵称 <em>必填</em><input value={profile.name} onChange={e=>setProfile({...profile,name:e.target.value})}/></label><label>我的生日 <small>选填</small><input value={profile.birthday} onChange={e=>setProfile({...profile,birthday:e.target.value})}/></label><label>当前城市 <small>可随时临时切换</small><input value={profile.city} onChange={e=>setProfile({...profile,city:e.target.value})}/></label><p className="form-section-label partner-label">TA 的资料</p><label>TA 的昵称 <em>必填</em><input value={partnerProfile.name} onChange={e=>setPartnerProfile({...partnerProfile,name:e.target.value})}/></label><label>TA 的生日 <small>选填</small><input value={partnerProfile.birthday} onChange={e=>setPartnerProfile({...partnerProfile,birthday:e.target.value})}/></label></section><button className="primary-button" disabled={!profile.name.trim()||!partnerProfile.name.trim()} onClick={()=>go("connect")}>保存双方资料并建立关系 <Arrow/></button></div>}
            ...
            {screen === "home" && (
              <div className="page tab-page">
                <div className="home-hero">
                  <div className="home-top"><button className="couple-avatars avatar-button" onClick={() => go("profile")} aria-label="查看我们的资料"><div className="avatar a">{profile.name.slice(0,1)}</div><div className="avatar b">{partnerProfile.name.slice(0,1)}</div></button><button className="round-button" onClick={() => go("settings")}>•••</button></div>
            ...
            {screen === "calendar" && (
              <div className="page tab-page calendar-page">
                ...
                <div className="month-grid">{Array.from({length: 35}, (_, i) => { const d = i - 1; const valid=d>0&&d<=31; return <button key={i} onClick={()=>valid&&setSelectedDay(d)} className={`${d === selectedDay ? "selected-day" : ""} ${d === 8 ? "today" : ""} ${d === 8 && adopted ? "official" : ""} ${d === 16 && hasGenerated ? "idea" : ""} ${d===15&&importantAdded?"important-dot":""} ${restDays.includes(d)?"rest-day":""} ${adjustedWorkDays.includes(d)?"work-day":""}`}><span>{valid ? d : ""}</span>{restDays.includes(d)&&<small className="day-type rest">休</small>}{adjustedWorkDays.includes(d)&&<small className="day-type work">班</small>}{d===15&&importantAdded&&<small>纪念日</small>}</button>; })}</div>
            ...
            {screen === "settings" && <div className="page tab-page formal-page settings-page"><header><div><p className="kicker">恋爱日记</p><h2>设置</h2></div><i/></header><section className="settings-profile" onClick={() => go("profile")}><div className="avatar a">{profile.name.slice(0,1)}</div><div><b>{profile.name}</b><small>与{partnerProfile.name}已连接</small></div><i>›</i></section><section className="settings-group"><SettingRow icon="♢" label="我们的资料" onClick={() => go("profile")}/><SettingRow icon="◌" label="重要日子" onClick={() => go("important")}/></section><section className="settings-group"><SettingRow icon="♢" label="通知与提醒" onClick={() => go("notifications")}/><SettingRow icon="◉" label="隐私与 AI 数据说明" onClick={() => go("privacy")}/><SettingRow icon="▢" label="照片与存储" onClick={() => notify("此公开原型仅在当前浏览器保存演示状态")}/></section><section className="settings-group"><SettingRow icon="?" label="帮助与反馈" onClick={() => notify("帮助中心将在产品开发阶段接入")}/><SettingRow icon="○" label="关于恋爱日记" value="V1.6"/></section>{bottomNav("settings")}</div>}
      ...
      {["profileEdit","reconnect","clearData"].includes(panel) && <div className="modal-backdrop" onClick={()=>setPanel("")}><section className="bottom-sheet" onClick={e=>e.stopPropagation()}><div className="sheet-handle"/>{panel==="profileEdit"&&<><p className="kicker">编辑双方资料</p><h2>共同资料，需要双方真实确认</h2><label>昵称<input value={profile.name} onChange={e=>setProfile({...profile,name:e.target.value})}/></label><label>生日<input value={profile.birthday} onChange={e=>setProfile({...profile,birthday:e.target.value})}/></label><label>当前城市<input value={profile.city} onChange={e=>setProfile({...profile,city:e.target.value})}/></label><label>TA 的昵称<input value={partnerProfile.name} onChange={e=>setPartnerProfile({...partnerProfile,name:e.target.value})}/></label><label>TA 的生日<input value={partnerProfile.birthday} onChange={e=>setPartnerProfile({...partnerProfile,birthday:e.target.value})}/></label><button className="primary-button" onClick={()=>{setPanel("");notify("双方资料已保存，并同步更新相关页面");}}>保存修改 <Arrow/></button></>}{panel==="reconnect"&&<><div className="danger-symbol">!</div><h2>重新建立关系？</h2><p className="sheet-copy">当前安排、任务和回忆仍保留在本机，但将不再与原关系相连。你需要重新邀请 TA。</p><button className="danger-button" onClick={()=>{setPanel("");setHasStarted(false);history.current=[];go("connect",true);}}>确认并重新连接</button><button className="ghost-button" onClick={()=>setPanel("")}>保留当前关系</button></>}{panel==="clearData"&&<><div className="danger-symbol">!</div><h2>清空当前浏览器数据？</h2><p className="sheet-copy">将删除本机保存的资料、条件、安排、任务与回忆演示状态。公开网站本身不会被删除，且此操作无法撤销。</p><button className="danger-button" onClick={()=>{window.localStorage.removeItem("love-diary-v16");window.localStorage.removeItem("love-diary-v15");window.localStorage.removeItem("love-diary-v14");setPanel("");setHasStarted(false);setAdopted(false);setPartnerAccepted(false);setCompleted(false);setTaskAccepted(false);setTaskLinked(false);setMemoryCreated(false);setImportantAdded(false);history.current=[];go("welcome",true);}}>确认清空并重新开始</button><button className="ghost-button" onClick={()=>setPanel("")}>取消</button></>}</section></div>}
...
function MultiChoice({ title, options, values, setValues }: { title: string; options: string[]; values: string[]; setValues: (v: string[]) => void }) {
  return <section className="choice-group"><h3>{title}</h3><div>{options.map(option => <Pill key={option} active={values.includes(option)} onClick={() => values.includes(option) ? setValues(values.filter(v=>v!==option)) : values.length < 2 ? setValues([...values,option]) : setValues([values[1],option])}>{option}</Pill>)}</div><p className="choice-help">已选择 {values.length}/2 · 选择第三项时会替换最早选择</p></section>;
}
