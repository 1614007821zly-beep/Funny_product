          ...
          <div className={`screen screen-${screen} ${hasRelationship?"":"solo-mode"}`}>
            ...
            {screen === "plan" && (
              <div className="page detail-page">
                ...
                <section className="plan-introduction"><p className="kicker">完整方案介绍</p><h3>为什么会推荐这个灵感</h3><p>{currentPlan.desc}</p><div><b>推荐依据</b><p>{hasRelationship?"结合你们":"结合你"}选择的“{myStates.join("、") || choices.mood}”、{choices.vibe}氛围、{choices.time}和{choices.budget}总预算进行筛选。</p></div><div><b>地点与体验</b><p>{currentPlace?`以高德真实地点“${currentPlace.name}”为核心，优先控制在 ${locationPrefs.radius/1000} 公里搜索范围内。`:`当前方案尚未匹配到可核验的真实地点，可返回调整商圈或搜索范围。`}</p></div><div><b>采用之后</b><p>这仍是候选灵感。确认日期和时间后才会进入日历；{hasRelationship?"共同安排仍需双方分别确认。":"你可以继续保存为个人计划。"}</p></div></section>
