...
type Place = { id: string; name: string; address: string; location: string; type: string; distance: number | null; businessArea: string; rating: string; cost: string; openTimeToday: string; category: string; recommendationReasons: string[]; verifiedBy: "amap" };
...
export default function Home() {
  ...
  const currentBudgetText = placeBudgetText(currentPlace, hasRelationship, choices.budget);
  ...
  const inspirationWeather = choices.time === "周末" ? weather?.forecasts.find(day => [0,6].includes(localDate(day.date).getDay())) ?? null : ["现在出发","今晚"].includes(choices.time) ? weather?.forecasts.find(day => day.date === dateInputValue(0)) ?? null : null;
  const scheduleWeather = weather?.forecasts.find(day => day.date === scheduleDraft.date) ?? null;
  ...
  const calendarWeather = weather?.forecasts.find(day => day.date === selectedCalendarDateKey) ?? null;
  ...
  async function generate(shouldFail = false) {
    ...
    try {
      ...
      const data = await response.json() as { plans?: Array<{ title: string; summary: string; duration: string; budgetLabel: string; timeline: TimelineNode[]; places?: Place[] }>; weather?: WeatherForecast | null; error?: string; code?: string };
      ...
      setAiPlans(data.plans.map((plan, index) => ({ eyebrow: index === 0 ? `主方案 · ${data.code === "REAL_PLACE_FALLBACK" ? "真实地点推荐" : "AI 实时生成"}` : `备选 · ${data.code === "REAL_PLACE_FALLBACK" ? "真实地点推荐" : "AI 实时生成"}`, title: plan.title, meta: `${choices.time} · ${choices.budget}`, desc: plan.summary, tone: ["primary", "cream", "lilac"][index] ?? "cream", duration: plan.duration, timeline: plan.timeline, places: plan.places })));
  ...
  async function uploadMemoryPhoto(file: File) {
    ...
    setMediaBusy(true);
    try {
      const form = new FormData(); form.set("file", file); form.set("visibility", hasRelationship ? "shared" : "personal");
      const response = await fetch("/api/media", { method: "POST", body: form });
      const data = await response.json() as { media?: UploadedMedia; error?: string };
      if (!response.ok || !data.media) throw new Error(data.error || "照片上传失败。");
      setUploadedMedia(data.media); setMemoryPhoto(true); setMemoryContentRetracted(false); notify("照片已安全上传");
    } catch (error) { notify(error instanceof Error ? error.message : "照片上传失败。"); }
    finally { setMediaBusy(false); }
  }

  async function retractUploadedMedia() {
    if (!uploadedMedia) { setMemoryContentRetracted(true); setMemoryPhoto(false); setPanel(""); return; }
    setMediaBusy(true);
    try {
      ...
      setUploadedMedia(null); setMemoryPhoto(false); setMemoryContentRetracted(true); setPanel(""); notify("照片已撤回，在线文件已删除");
  ...
  useEffect(() => {
    ...
    const city = ["confirm", "schedule"].includes(screen) ? scheduleDraft.city || profile.city : inspirationCity;
    window.queueMicrotask(() => void loadWeather(city, true));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [screen, account?.authenticated, inspirationCity, profile.city, scheduleDraft.city]);
          ...
          <div className={`screen screen-${screen} ${hasRelationship?"":"solo-mode"}`}>
            ...
            {screen === "results" && (
              <div className="page result-page">
                ...
                <div className="result-intro"><p className="kicker">{inspirationCity} · {choices.time} · {choices.mood} · {choices.budget}</p><h2>{choices.space === "室内" ? <>留在室内，<br/>也能认真约会。</> : <>不赶时间，<br/>也不辜负今晚。</>}</h2>{inspirationWeather&&<p className="weather-summary">高德天气 · {inspirationWeather.date} · {weatherLabel(inspirationWeather)}</p>}</div>
                <div className="plan-stack"><button className={`plan-card primary main-plan ${selectedPlan === 0 ? "chosen" : ""}`} aria-pressed={selectedPlan === 0} onClick={() => setSelectedPlan(0)}><div className="plan-top"><span>主灵感 · {aiPlans ? "真实地点组合" : "演示方案"}</span>{selectedPlan === 0 && <i>✓ 当前方案</i>}</div><h3>{dynamicPlans[0].title}</h3><p className="plan-meta">{choices.time} · {choices.budget} · {choices.space}</p><p>{dynamicPlans[0].desc}</p>{dynamicPlans[0].places?.[0]&&<span className="place-preview"><b>真实地点</b>{dynamicPlans[0].places[0].name} · {dynamicPlans[0].places[0].recommendationReasons[0]}</span>}{choices.special.trim()&&<span className="prep-note">特别照顾：{choices.special.trim()}</span>}</button><div className="alternative-title"><b>也可以试试</b><button onClick={() => {setSelectedPlan((selectedPlan+1)%3);notify("已将下一方案设为主方案");}}>换一个</button></div>{dynamicPlans.slice(1).map((plan, offset) => {const i=offset+1;return <button key={plan.title} className={`plan-card alternative ${selectedPlan === i ? "chosen" : ""}`} aria-pressed={selectedPlan === i} onClick={() => setSelectedPlan(i)}><div><h3>{plan.title}</h3><p className="plan-meta">{plan.meta}</p>{plan.places?.[0]&&<p className="alternative-place">{plan.places[0].name}</p>}</div><i aria-hidden="true">›</i></button>})}</div>
            ...
            {screen === "plan" && (
              <div className="page detail-page">
                <div className="detail-hero"><header><Back onClick={() => back("results")}/><span>AI 详细计划</span><button className="icon-button" onClick={() => notify("为保护灵感条件，当前版本不会生成公开计划链接")} aria-label="分享计划说明">↗</button></header><p className="kicker">候选方案 · 尚未进入日历</p><h2>{currentPlan.title}</h2><p className="plan-summary">{currentPlan.desc}</p><div className="detail-meta"><span>{inspirationCity} · {choices.time}</span><span>{currentPlan.duration || "约 3.5 小时"}</span><span>{choices.budget}</span></div></div>
            ...
            {screen === "schedule" && scheduleLoaded && sharedSchedule && (
              <div className="page schedule-page">
                ...
                <section className="schedule-card"><div><span className="label">时间</span><b>{eventMonthDay} {scheduleDraft.time}</b></div><div><span className="label">集合</span><b>近江地铁站 B 口</b></div><div><span className="label">天气</span><b>{scheduleWeather?weatherLabel(scheduleWeather):"仅显示未来 4 天预报"}</b></div><div><span className="label">预算</span><b>以确认安排时为准</b></div><button onClick={() => go("plan")}>查看完整路线 <span>›</span></button></section>
            ...
            {screen === "memory" && (
              <div className="page memory-page">
                <div className={`memory-cover ${memoryPhoto&&!memoryContentRetracted ? "with-photo" : ""}`} style={memoryPhoto&&uploadedMedia&&!memoryContentRetracted?{backgroundImage:`linear-gradient(#3a38415e,#5b3b32cc),url(${uploadedMedia.url})`}:undefined}><header><Back onClick={() => back("memories")}/><span>回忆详情</span><button className="icon-button" onClick={() => notify("回忆公开分享需先完成双方内容授权，当前仅支持安排分享")} aria-label="分享回忆">↗</button></header><div className="moon">☽</div><div className="city-lights">•• · • ·• ••</div><div className="cover-copy"><p>{eventDateLong} · {profile.city}</p><h2>{hasRelationship?<>晚风里，<br/>我们聊了很久。</>:<>{memoryDraft.title}</>}</h2></div></div>
            ...
            {screen === "about" && <div className="page formal-page about-page"><header><Back onClick={() => back("settings")}/><span>关于恋爱日记</span><i aria-hidden="true"/></header><div className="about-mark">♡</div><section className="page-intro compact"><p className="kicker">V1.17 · 第四阶段</p><h2>让共同生活，<br/>更容易被认真对待。</h2><p className="confirm-copy">恋爱日记帮助两个人从灵感走到正式安排，再把真实发生的事自然留成回忆。AI 只提供建议，不替任何人确认事实或评价关系。</p></section><section className="info-group"><InfoRow label="当前版本" value="V1.17"/><InfoRow label="数据原则" value="个人所有 · 双方确认"/><InfoRow label="节假日数据" value="国务院办公厅年度通知"/></section><a className="secondary-button map-link" href={holidaySource.url} target="_blank" rel="noreferrer">查看 2026 年节假日来源 <Arrow/></a></div>}
...
function PlaceCandidates({ places, selectedId, onSelect }: { places: Place[]; selectedId?: string; onSelect: (index: number) => void }) {
  ...
  return <section className="place-candidates" aria-labelledby="place-candidates-title"><div className="section-heading"><div><p className="kicker">附近候选</p><h3 id="place-candidates-title">选择更合适的地点</h3></div><span>{places.length} 个真实地点</span></div>{places.length ? places.map((place, index) => <button type="button" key={place.id} className={selectedId === place.id ? "selected" : ""} aria-pressed={selectedId === place.id} onClick={() => onSelect(index)}><div><b>{place.name}</b><small title={place.businessArea || place.address}>{place.businessArea || place.address}</small><em>{place.recommendationReasons.slice(0,2).join(" · ")}</em></div><span>{place.distance !== null ? `${distanceFormatter.format(place.distance / 1000)}\u00a0公里` : "距离待定位"}</span></button>) : <p className="candidate-empty">附近没有匹配到地点，请扩大范围或调整商圈。</p>}</section>;
}

function placeBudgetText(place: Place | null, hasRelationship: boolean, budget: string) {
  const cost = Number(place?.cost);
  if (!Number.isFinite(cost) || cost <= 0) return `总预算 ${budget} · 地点价格待确认`;
  return `${hasRelationship ? "两人地点" : "地点"}参考约 ¥${Math.round(cost * (hasRelationship ? 2 : 1))} · 总预算 ${budget}`;
}
