...
export default function Home() {
  ...
  function clearLocalSession() {
    ...
    setLegacyPlan(null); setSoloMode(false); setAiPlans(null); setMorePlans([]); setSeenPlaceIds([]); setRecommendationFeedback(emptyRecommendationFeedback()); setHasGenerated(false); setTaskContextActive(false); setTaskLinked(false); setMemoryContentRetracted(false); setRelationshipExited(false); setSafetyExitUsed(false); setPanel("");
  ...
  return (
    <main className="prototype-shell" id="main-content">
      <aside className="prototype-notes">
        ...
        <p className="kicker">恋爱日记 · V51 单人与共同体验</p>
        ...
        <p className="hint">V51 单人内容默认私密；共同任务和重要日子必须由双方确认。</p>
      </aside>
      ...
      <section className="phone-stage">
        <div className="phone">
          ...
          <div className={`screen screen-${screen} ${hasRelationship?"":"solo-mode"}`}>
            ...
            {screen === "about" && <div className="page formal-page about-page"><header><Back onClick={() => back("settings")}/><span>关于恋爱日记</span><i aria-hidden="true"/></header><div className="about-mark">♡</div><section className="page-intro compact"><p className="kicker">V51 · 完整安排与回忆</p><h2>让共同生活，<br/>更容易被认真对待。</h2><p className="confirm-copy">恋爱日记帮助两个人从灵感走到正式安排，再把真实发生的事自然留成回忆。AI 只提供建议，不替任何人确认事实或评价关系。</p></section><section className="info-group"><InfoRow label="当前版本" value="V51"/><InfoRow label="数据原则" value="个人所有 · 双方确认"/><InfoRow label="节假日数据" value="国务院办公厅年度通知"/></section><a className="secondary-button map-link" href={holidaySource.url} target="_blank" rel="noreferrer">查看 2026 年节假日来源 <Arrow/></a></div>}
